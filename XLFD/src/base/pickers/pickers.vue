<template>
  <div id="h-picker">
    <div class="picker-background" v-show="show"  @click="close"></div>
          ...
          <div class="area_roll">
            <div>
              <div top="0"
                ...
                <div class="tooth" v-for="(item,index) in pData1" :key="index">{{item[txtKey]}}</div>
              </div>
              <div class="area_grid">
              </div>
            </div>
            <div v-if="columns > 1">
              <div
                  ...
                  @touchend="gearTouchEnd"
                  val="5">
                <div class="tooth" v-for="(item,index) in pData2" :key="index">{{item[txtKey]}}</div>
              </div>
              <div class="area_grid">
              </div>
            </div>
            <div v-if="columns > 2">
              <div
                  ...
                  @touchend="gearTouchEnd"
                  val="5">
                <div class="tooth" v-for="(item,index) in pData3" :key="index">{{item[txtKey]}}</div>
              </div>
              <div class="area_grid">
              </div>
            </div>
  ...
  methods: {
    ...
    setTop(defaultData) {
      this.$nextTick(() => {
        ...
        var pos2 = 0;
        var pos3 = 0;
        if (defaultData[0] && defaultData[0][this.valueKey]) {
          this.selects.select1 = defaultData[0];
          for (var i = 0, len = this.pData1.length; i < len; i++) {
            if (this.pData1[i][this.valueKey] == defaultData[0][this.valueKey]) {
              pos1 = -(i * 2);
              break;
            }
          }
        ...
        if (defaultData[1] && defaultData[1][this.valueKey]) {
          if (this.link) {
            this.pData2 = this.selectData.data2[defaultData[0][this.valueKey]];
          }
          for (var i = 0, len = this.pData2.length; i < len; i++) {
            if (this.pData2[i][this.valueKey] == defaultData[1][this.valueKey]) {
              pos2 = -(i * 2);
              break;
            }
          }
        ...
        if (defaultData[2] && defaultData[2][this.valueKey]) {
          if (this.link) {
            this.pData3 = this.selectData.data3[defaultData[1][this.valueKey]];
          }
          for (var i = 0, len = this.pData3.length; i < len; i++) {
            if (this.pData3[i][this.valueKey] == defaultData[2][this.valueKey]) {
              pos3 = -(i * 2);
              break;
            }
          }
    ...
    resetData2(endVal) {
      this.$nextTick(() => {
        ...
        if (
          this.pData1[endVal] &&
          this.selectData.data2[this.pData1[endVal][this.valueKey]]
        ) {
          this.pData2 = this.selectData.data2[this.pData1[endVal][this.valueKey]];
        } else {
          this.pData2 = [];
        }
    ...
    resetData3(endVal) {
      this.$nextTick(() => {
        ...
        if (this.pData2.length > 0 && this.pData2[endVal]) {
          this.pData3 = this.selectData.data3[this.pData2[endVal][this.valueKey]];
        } else {
          this.pData3 = [];
        }
    ...
    init() {
      if (!this.link) {
        ...
        this.pData1 = this.selectData.data1;
        this.pData2 = this.selectData.data2[this.pData1[0][this.valueKey]];
        if (this.columns === 3) {
          this.pData3 = this.selectData.data3[this.pData2[0][this.valueKey]];
        }
      }
      if (this.columns === 1) {
