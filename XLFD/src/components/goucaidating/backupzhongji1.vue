        ...
        intervlRunCount(sub,num,subnum){      
            setInterval(() => {
                this.truetotalList.map((v,k)=>{
                    for(let i=0;i<v.trueSubList.length;i++){                       
                        if(v.trueSubList[i].locktime=="00:00:00"){
                            v.trueSubList[i].locktime="--:--:--"
                           this.getSingleLockTime(v.trueSubList[i],k,i)
                        }
                        else if(v.trueSubList[i].locktime!="--:--:--"){
                            v.trueSubList[i].locktime=countTime(v.trueSubList[i].lock_time.replace(/-/g,'/'));
                        }
                    }
                })   
            },1000);
